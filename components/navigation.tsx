...
export function Navigation() {
  ...
  return <header className="fixed inset-x-0 top-0 z-50 px-4 pt-4 md:px-7 md:pt-6">
    <div className="mx-auto flex max-w-[1440px] items-center justify-between rounded-full border border-white/10 bg-[#121b21]/85 px-4 py-3 backdrop-blur-md md:px-5">
      ...
      {open && <div className="absolute left-0 right-0 top-[58px] rounded-2xl border border-white/10 bg-[#121b21] p-3 md:hidden">{links.map(link => <Link onClick={() => setOpen(false)} className="mono block rounded-lg px-3 py-3 text-xs uppercase text-white/70 hover:bg-white/5" key={link.href} href={link.href}>{link.label}</Link>)}</div>}
    </div>
  </header>;
}
