...
export function PortfolioTabs({ creatives, logos, caseStudies }: { creatives: PortfolioItem[]; logos: string[]; caseStudies: PortfolioItem[] }) {
  ...
  return <section className="px-5 pb-24 pt-0 md:px-8 md:pt-0"><div className="mx-auto max-w-[1440px]">
    ...
    {active === "Clientele" && <div className="mt-12 grid grid-cols-2 gap-3 md:grid-cols-4">{logos.map((title, index) => <article key={`${title}-${index}`} className="logo-tile group min-w-0"><div className={`logo-tile-frame ${[2, 5, 6, 7, 8, 10, 11, 15].includes(index) ? "logo-tile-frame-dark" : ""}`}><FlexibleAssetImage src={`/assets/logos/logo-${String(index + 1).padStart(2, "0")}.png`} alt={title} className="absolute inset-0 h-full w-full object-contain p-5 transition duration-500 group-hover:scale-105"/></div></article>)}</div>}
